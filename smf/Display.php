...
.gpbp_voted_down {
	...
	if ($context['is_poll'])
	{
		...
		$request = $smcFunc['db_query']('', '
			SELECT pc.id_choice, pc.label, pc.votes, IFNULL(lp.id_choice, -1) AS voted_this, pc.requires_comment
		...
		$vvisTxt = array(
			...
			array('label' => 'poll_vv_opt_discrete', 'desc' => 'poll_vv_opt_discrete_txt'),
			array('label' => 'poll_vv_opt_public', 'desc' => 'poll_vv_opt_public_txt')
		);
		
		// append a visibility warning to the warning string
		$context['poll']['allowed_warning'] .= '  '.sprintf($txt['poll_vv_vlog_vote_warning'], $txt[$vvisTxt[$pollinfo['vote_visibility']]['label']], $txt[$vvisTxt[$pollinfo['vote_visibility']]['desc']]);
		...
		foreach ($pollOptions as $i => $option)
		{
			...
			$context['requires_comments'] |= ($option['requires_comment'] == 1);
			...
			$context['poll']['options'][$i] = array(
				...
				'requires_comment' => $option['requires_comment'],
