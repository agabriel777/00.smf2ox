...
CREATE
    ...
    TRIGGER `ow_forum_post_after_insert` AFTER INSERT ON `ow_forum_post` 
    FOR EACH ROW
...
INTO v_id_topic, v_id_board
FROM smf_topics t 
WHERE t.ow_id=NEW.topicID;

SELECT 'subject' INTO v_subject 
FROM smf_messages WHERE id_topic = v_id_topic
LIMIT 1;


SELECT id_member, member_name,  email_address
INTO v_id_member, v_poster_name, v_poster_email
FROM smf_members WHERE ow_id = NEW.userId;

INSERT INTO `vaspun`.`smf_messages` (
  `id_topic`, `id_board`, `poster_time`,
  `id_member`, `id_msg_modified`,  `subject`,
  `poster_name`, `poster_email`,
  `poster_ip`,  `smileys_enabled`,
  `modified_time`,  `modified_name`,
  `body`, `icon`, `approved`, `gpbp_score`, `ow_id`) 
VALUES
  (
    v_id_topic, v_id_board, NEW.createStamp,
    v_id_member, 0, v_subject,
    v_poster_name,  v_poster_email,
    0, 1,  0, '', -- 'Guest',
    v_body, 'xx', 1, 0, v_ow_id) ;
