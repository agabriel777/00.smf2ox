...
function get_topic_id($link, $old_id) {
	...
	$q = "select ow_id from smf_topic where id_topic=".$old_id;
	$result = mysqli_query($link, $q);
	$row = mysqli_fetch_array($result);
	return $row['ow_id'];
}
...
function import_messages ($link) //from topics
{
  ...
  $qsmf = "select * from smf_messages order by id_msg";
	...
	while($row = mysqli_fetch_array($result))
  {
	 ...
	 $qIns.= "(".$id.", ".get_topic_id($link, $row['id_topic']).", ".get_user_id($link, $row['id_member']).", '".$row['body']."', ".$row['poster_time'].")";
